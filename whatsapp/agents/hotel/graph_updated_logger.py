    ...
    if next_step == "ask_room":
        ...
        if booking.get("check_in") and booking.get("check_out"):
            ...
            def format_amenities(amenities):
                if not amenities: return "standard"
                if isinstance(amenities, str): return amenities
...
def build_graph():
    ...
    for node in [
        "greeting", "small_talk", "control", "search_rooms",
        "make_booking", "cancel_booking", "check_booking",
        "check_prices", "rag", "expand",
    ]:
